	...
	public class Solver
	{
		...
		private Point? DropSand(int yMax, char[][] array, Point pt)
		{
			...
			if (array[nextPoint.Y][nextPoint.X] == '#' || array[nextPoint.Y][nextPoint.X] == 'o')
			{
				...
				{
					if (nextPoint.X + 1 > array[nextPoint.Y].Length)
					{
						return null;
					}
		...
		private (int Normalize, int YMax, char[][] Array) Parse()
		{
			...
			var minX = rockPoints.MinBy(x => x.X).X;
			var maxY = rockPoints.MaxBy(x => x.Y).Y;
			...
			for (var y = 0; y <= maxY; ++y)
			{
				...
				{
					arr[y][x] = rockPoints.Contains(new Point(x + minX, y)) ? '#' : '.';
				}
			}
